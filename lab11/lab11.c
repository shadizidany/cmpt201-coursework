...
#define MAX_FILE_SIZE 512
  ...
  size_t msglen = read_all_bytes(message_path, message, sizeof(message));
  size_t siglen = read_all_bytes(sign_path, signature, sizeof(signature));
  ...
  EVP_DigestVerifyUpdate(mdctx, message, msglen);
  int rc = EVP_DigestVerifyFinal(mdctx, signature, siglen);
