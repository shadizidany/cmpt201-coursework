...
port number). 127.0.0.1 port 8000
2. Is it UDP or TCP? How do you know?
  TCP because 2nd argument to socket() is SOCK_STREAM
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? From stdin since it's using
read(STDIN_FILENO, ...) to store data into buf and then use buf as argument to
write()
4. How does the client program end? How can you tell that in the code?
  If any of the syscalls (like read/write) failed due to error then program
terminates by running handle_error macro
*/
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
